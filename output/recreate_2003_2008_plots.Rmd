...
pm25_fb_bind <- merge(pm25_bind, fb_bind[,c("date", "oc_r", "poc_t", "station")], 
...
There are differences in the number of observations for all cities. Still need 
to investigate why. 
```{r, echo = FALSE, message = FALSE}
#------------------------------------------------------------------------------
#------------------------------------------------------------------------------
...
Most evident difference from the paper is passive travel and field blank (blue)
- still working on identifying the correct passive travel variable in the data.
...
oc_station <- spec_data %>%
  ...
  mutate(passiveTravel_fieldBlank = oc_r) %>% # + ocec_cart_b_poc_r) %>%
  dplyr::select("city",
                "mean_oc_cart_a",
                "mean_oc_cart_b",
...
comp_station <- spec_data %>%
  select('city','date', 'month',edxrf_silicon_si, edxrf_calcium_ca, edxrf_iron_fe,  
         edxrf_titanium_ti, edxrf_potassium_k, speciation_mass_ug_m3, dich_pm2_5)
...
Statement is not consistent with my findings, mean total mass for those sites
...
\newpage 
comp_station_orig <- comp_station_orig[with(comp_station_orig, order(date, city)), ]
# Figure 9: Median soil and sodium chloride concentrations by site and month 
Issue with Simcoe data point 
