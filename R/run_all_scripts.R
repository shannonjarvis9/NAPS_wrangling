...
all_scripts <- grep("run_all_scripts|0", list.files(wd$R), invert = TRUE, value = TRUE)
...
if(response == "yes" | response == "y") {
  ...
  while (response %in% c("yes", "y", "no", "n")) {
    response <-
      readline(prompt = "Invalid response. Do you want to downlaod the NAPS data (~ 1 hr)? (y/n) ")
  }
  if (response == "yes" | response == "y") {
    break
  } else if (response == "no" | response == "n") {
