...
pm25_fb_bind <- merge(pm25_bind, fb_bind[,c("date", "spec_poc_fb", "spec_oc_r", "station")], 
...
ggplot(data = fig2, mapping = aes(x = city, y = spec_pm2_5, ymin = min,
       ymax = max)) + 
  geom_boxplot() +
  ylim(0, 80) + 
...
oc_station <- spec_data %>%
  ...
  mutate(passiveTravel_fieldBlank = spec_poc_fb + spec_oc_r) %>%
  dplyr::select("city",
                "mean_oc_cart_a",
                "mean_oc_cart_b",
...
comp_station <- spec_data %>%
  select('city','date', 'month', ions_ic_media_t_nitrate, ions_ic_media_t_sulphate, ions_ic_media_t_ammonium,
         ...
         ammonia_ic_cart_c_ammonia, spec_pm2_5)
