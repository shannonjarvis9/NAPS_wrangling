...
removeEmptyRowsCols <- function(dt_frm){
  dt_frm[!all(is.na(dt_frm[,2:ncol(dt_frm)])), ] 
}
...
getNumericalCols <- function(dt_frm){
  # vector of columns names that should not be numeric 
...
##         SamplerInfo: description and code of sampler (to identify as spec/dich)
...
readData_xlsx_sheets <- function(dir){
  ...
  # Itterate through the files, adding to the list by station number
...
readData_xls_nosheets <- function(dir){
  ...
  # itterate through the files, adding them to a list 
...
getSamplerInfo <- function(dir){
  ...
  # Itterate through the files, looking for the row that has the sampler info
  for(i in 1:length(files)){
